<div id="divllenar">
	<?if(isset($direct_comercio)){
		foreach($direct_comercio as $row){?>
		<input type="hidden"  id="redirent_comercio"  cod_tipo_comercio="<?=$row->idTipoComercio?>"  id_comercio="<?=$row->id_comercio?>" nombre_comercio="<?=$row->nombre?>" tipo_comercio="<?=$row->tipoComercio ?>"/>
	<?}}?>
  ...
  <div class="col-md-3 col-sm-4 col-6" style="margin-bottom: 1em" >
     <div class="thumbnail"  id="tarjeta_comercio" class="tarjeta_comercio_<?=$row->idTipoComercio?>" cod_tipo_comercio="<?=$row->idTipoComercio?>"  id_comercio="<?=$row->id_comercio?>" nombre_comercio="<?=$row->nombre?>" tipo_comercio="<?=$row->tipoComercio ?>">
