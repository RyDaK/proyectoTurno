...
class PedidosOP extends MY_Controller {
	...
	public function buscar_usuario(){
		...
		if($data->dni == ''){
			$result['result']=0;
			$result['data']['msg']= '<p class="p-info"><i class="fa fa-info-circle"></i> Debe ingresar un DNI.</p>';
		...
		if(count($rs) < 1){
			$result['result']=0;
			$result['data']['msg']= '<p class="p-info"><i class="fa fa-info-circle"></i> No se encontró ningún usuario con el Documento: <b>'.$data->dni.'</b>.</p>';
			echo json_encode($result);
			exit();
	...
	public function para_el_carrito(){
		...
		foreach($array['data'] as $row){
			...
			$html .= "<tr>";
				...
				$html .= "<td  art_cod='".$row->id_articulo."' id='td_precio".$i."'>".$row->precio."</td>";
		...
		if($rs){
			...
			$cod_ped_det = $this->db->insert_id();
				...
				foreach($query->result() as $row){
					...
					$query = $this->db->get_where('ms_metodo_pago',array('nombre'=>$medio_pago));
					foreach($query->result() as $row){
						$cod_medio_pago = $row->id_metodo_pag;
			...
			if($data->cod_etapa_ped == 2 || $data->cod_etapa_ped == 4){
				...
				if($data->cod_etapa_ped == 2){
					...
					$envio = enviar_sms('NOTIFICACION TURNO','Estimado '.$config['nombres_usuario'].' su pedido ha sido aceptado por '.$config['razon_social'],'51'.$telefono_usuario);
					
					
					if($medio_envio_telf != '') $envio = enviar_sms('NOTIFICACION TURNO',$mensaje_motorizado,'51'.$medio_envio_telf);
				}
				if($data->cod_etapa_ped == 4){
					$envio = enviar_sms('NOTIFICACION TURNO',$mensaje_cliente,'51'.$telefono_usuario);
					// if($medio_envio_telf != '') $envio = enviar_sms('NOTIFICACION TURNO',$mensaje_motorizado,'51'.$medio_envio_telf);
				}
			}
	...
	public function agregar_pedido_op(){
		...
		if(($query->num_rows() > 0)){
			...
			if(($query->num_rows() > 0)){
				$config_ = array( 'type' => 2, 'message' => "El Telefono ya pertenece a otro usuario.");
				$result['data']['content']=createMessage($config_);
				$result['status']=0;
				echo json_encode($result);
				exit();
		...
		for ($i=0; $i <= $data['bucle']; $i++) { 
			...
			$importe = $data['articulo'.$i] * $data['cantidad'.$i];
